...
function ExpiredCertifications() {
  ...
  const loadExpiredCertifications = () => {
    ...
    allCerts.forEach(cert => {
      ...
      if (days < 0) {
        expired.push({...cert, daysExpired: Math.abs(days)});
      } else if (days <= 30) {
  ...
  return (
    <div style={styles.pageContainer}>
      ...
      <div style={styles.container}>
        <h1 style={styles.title}>⚠️ Expired & Expiring Certifications</h1>
        <p style={styles.subtitle}>Monitor certifications that need attention</p>

        {/* Stats */}
        <div style={styles.statsWrapper}>
          <div style={{...styles.statCard, background: "#ff4757"}}>
            <h2 style={styles.statNumber}>{expiredCerts.length}</h2>
          ...
          <div style={{...styles.statCard, background: "#ffa502"}}>
            ...
            <p style={styles.statLabel}>Expiring Soon (30 days)</p>
          </div>
        </div>

        {/* Expired Certifications */}
        <div style={styles.section}>
          <h2 style={styles.sectionTitle}>❌ Expired Certifications</h2>
          {expiredCerts.length === 0 ? (
            <div style={styles.emptyCard}>
              <p>✅ No expired certifications!</p>
            </div>
          ) : (
            <div style={styles.grid}>
              {expiredCerts.map((cert, index) => (
                <div key={index} style={{...styles.card, borderLeft: "5px solid #ff4757"}}>
                  <div style={styles.urgentBadge}>
                    EXPIRED {cert.daysExpired} days ago
                  </div>
                  <h3 style={styles.certName}>🎓 {cert.name}</h3>
                  <div style={styles.certDetails}>
                    <p><strong>🏢 Issuer:</strong> {cert.issuer}</p>
                    <p><strong>📅 Expired On:</strong> {new Date(cert.date).toLocaleDateString()}</p>
                    {cert.file && <p><strong>📎 File:</strong> {cert.file}</p>}
                  </div>
                  <div style={styles.renewalAlert}>
        ...
        {/* Expiring Soon */}
        <div style={styles.section}>
          <h2 style={styles.sectionTitle}>⏰ Expiring Soon (Within 30 Days)</h2>
